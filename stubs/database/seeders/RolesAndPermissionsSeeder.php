...
class RolesAndPermissionsSeeder extends Seeder
{
    ...
    public function run(): void
    {
        $roles = config('arkhe.roles');

        foreach ($roles as $role => $permissions) {
            $role = Role::updateOrCreate(['name' => $role, 'label' => $role->label()]);

            if ($permissions === '*') {
                foreach (config('arkhe.permissions') as $name => $permissions) {
                ...
                foreach (config('arkhe.permissions.' . $permissions) as $permission) {
